...
class NanoporeRunConnection(statusdb_connection):
    def __init__(self, dbname='nanopore_runs'):
        ...
        self.db = self.connection[dbname]
        self.name_view = {k.key:k.id for k in self.db.view("names/name", reduce=False)}  #TODO: is this right?
        self.proj_list = {k.key:k.value for k in self.db.view("names/project_ids_list", reduce=False) if k.key} #TODO: is this right?