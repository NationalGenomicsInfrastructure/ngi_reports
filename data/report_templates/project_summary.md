...
{% if not project.is_finished_lib -%}
...
{% else %}
NGI ID | User ID | {% if project.not_as_million %}#{% else %}M{% endif %}reads | >=Q30(%) {% if not project.is_finished_lib %}| Status {% endif %}
-------|---------|--------|----------{% if not project.is_finished_lib %}|-------- {% endif %}
{% for sample in samples.values()|sort(attribute='ngi_id') -%}
{{ sample.ngi_id }} | {{ sample.customer_name }} | `{{ sample.total_reads }}` | {{ sample.qscore }} {% if not project.is_finished_lib %} | {{ sample.seq_status }} {% endif %}
{% endfor %}
